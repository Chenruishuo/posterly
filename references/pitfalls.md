...
16. **Equal padding need not look centered with lopsided font metrics.** Fonts with a large ascent (Charter is one example) can sit low in their line box, leaving `.callout`, `.eqn`, or tile text looking top-heavy even with equal padding. No gate measures ink: bias the padding (top ≈ 2/3 of bottom worked for Charter at line-height 1.3), or measure ink with canvas `actualBoundingBoxAscent` / `actualBoundingBoxDescent`, then re-render to check.
17. **A late `.ff-fig` width override defeats `.w-NN`.** An override block after the template utilities makes `.ff-fig { width: 46%; }` beat `.w-55` on the same element: equal specificity, later rule wins. Put width overrides on the intended combinations (`.ff-fig.w-40`, `.ff-fig.w-45`, `.ff-fig.w-50`, `.ff-fig.w-55`), or place the base override before the utilities.
