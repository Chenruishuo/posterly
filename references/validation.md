...
- `poster_check.py`:
  ...
  - `polish` — **soft** visual gate (figure sizing by AR, broken images, typography orphans, space-between fill, card trailing / mid-card voids, `<br>`-in-flex collapse, header logos: broken / oversized / QR mismatch / title squeeze). Warns by default; `--strict` to fail. `--widow-fill 0.5` sets the ordinary last-line fill threshold (fraction in `(0, 1]`, default 0.5; `.fb-text` stays at 0.80); `run_gates.py` forwards the same flag. After WARN lines, stdout lists up to 25 judged last-line fills in ascending order, including clean tails; this census is omitted from the orchestrator’s JSON summary. Hard-fails if the poster has no `[data-measure-role]` markup at all (silent PASS would be a worse bug). Its measurement half also rides `measure --with-polish` (same rendered page, advisory there); full standalone-polish coverage remains the loop's final soft gate, with an unchanged-version result reusable under [result reuse](#result-reuse).
