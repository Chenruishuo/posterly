...
**Lone wrapped fragment — prose that wraps to a short, stranded last line.** `polish` emits `WIDOW` when the last visual line fills **less than 50% of the widest typeset text line**, configurable with `--widow-fill` (default `0.5`). The width bar applies to `.callout`, `.body-text`, `.caption`, `.section-title`, `.card p`, `.card li`, and `.fb-text`, including each `<br>`-delimited segment, **regardless of block length**. Generic blocks discovered by geometry (including custom classes such as `.table-note`) get the same width bar when their tail has **at most four glue units**, covering one-/two-unit runts and short three-/four-unit tails without flooding on short custom labels. A generic block with **at least three visual lines and 20 text words** is also treated as prose and gets the width bar regardless of tail length; this covers explanatory notes whose short final line can still hold many small words. A unit is a text token or inline math expression; an `&nbsp;`-glued text pair counts as one. **Table cells keep their conservative rule:** from four prose words up, judge tails of at most two units against the width bar. Fix a cell with `text-wrap: balance`, shorter copy, or a wider column, then re-run `measure` because the row height may change.

**Framework-banner exception (`.fb-text`):** its last line must fill **at least 80%**, independent of `--widow-fill` and block length, so it reads as a filled rectangle. Reflow with a moderate combination of width, a one-step font-size adjustment, truthful expansion, or trimming to one fewer full line; protect stat-box space and avoid oversized type. Never force fill with justification or letter-spacing padding. Re-render and look: clearing the bar alone does not establish visual quality.

**Math and limits:** inline `mjx-container` (without `display="true"`), `.MathJax`, and `math` count as word-bearing content, even when the tail consists only of math plus punctuation (`95%).`, `S ≈ 95%.`); measure that tail’s full visual width. Opaque contents stay out of the prose text stream. A last line containing real media (`img`, `svg`, `canvas`, `table`), or display math without a prose word, keeps its skip. The gate also skips single-line segments, `white-space: nowrap/pre`, RTL/vertical text, and intentional `data-vrail-title` stacks. After the warnings, a **last-line fill census** prints the lowest 25 judged fills, including near-threshold tails that did not flag, for visual review.

**Fix after `measure` is green**, preferably keeping the block’s line count, and re-run the gates after reflow. For ordinary prose, first **reword** to make both the last line and the line above fill naturally; for a centered heading/title, use **`text-wrap: balance`**. `&nbsp;` glue is a **last resort**, appropriate for a leading footnote marker or tight stat cell: at most two tokens, never a chain. Glue cannot evade the width bar by increasing token count, and it can leave an early-wrap hole on the line above. `GLUE-CHAIN` flags ≥3 prose words fused with `&nbsp;` (stat/math/list idioms are exempt); use `box-decoration-break: clone` for a multi-word `.mark` across a wrap. Never ship a stranded short tail, a lone `*` / `†` / `‡` fragment, or a half-empty line created by a glue fix; **re-render and look** after every fix.
